...
export class ClovaService {
  ...
  async postchat(data: any, roomId: number): Promise<any> {
    // 기존 대화 가져오기
    ...
    previousConversations.forEach((conv) => {
      if (isValidJsonString(conv.messages.toString())) {
        ...
        if (Array.isArray(messages)) {
          combinedMessages = [...combinedMessages, ...messages];
        } else {
    ...
    // 새로운 메시지 추가
    combinedMessages.push({ role: 'user', content: data.messages });
  ...
  async getNextChatRoomId(userId: number, profileId: number): Promise<any> {
    ...
    const summaryResponse = await this.postchat(postMessage, newChatRoom.id);
  ...
  async summarizeOldestConversation(chatRoomId: number): Promise<string> {
    ...
    const summaryResponse = await this.postchat(postMessage, chatRoomId);
